...
echo '<h3 class="tb">' . get_user_or_websitename() . '</h3>';
 ...
 //Adding Top Bar Plugin Page on the side bar
 ...
 // Begin creating content/field on the top bar plugin page

 function topbar_register_setttings() {
    register_setting('topbar_option_group', 'topbar_field');
 }

 add_action('admin_init', 'topbar_register_setttings');
...
function topbarstyle() {
    ...
    .top-bar-wrapper label {font-size: 3m; display: block; line-height: normal; margin-bottom: 30px; font-weight: bold}
    .top-bar-wrapper input {color: #666; width: 100%; padding: 30px; font-size: 3em}
    .top-bar-wrapper .button {font-size: 2em; text-transform: uppercase; background: rgba(59, 173, 227, 1); background: linear-gradient(45deg, rgba(59,173,227,1) 0%, rgba(87, 111, 230, 1) 25%, rgba(152, 68, 183, 1) 51%, rgba(225,53,127,1) 100%); border:none}
    </style>';
}